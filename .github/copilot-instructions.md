...
A `dev.gitlive` member that only delegates to its `com.google.firebase` counterpart (at most with `.await()`) adds no value
and is deprecated with `DELEGATES_TO_ANDROID_SDK_API` and a `ReplaceWith` naming the counterpart, spelled with the
fully qualified `com.google.firebase.Firebase` object since both layers have a `Firebase` object.
