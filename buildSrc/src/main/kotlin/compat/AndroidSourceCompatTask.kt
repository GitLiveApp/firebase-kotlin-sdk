...
abstract class AndroidSourceCompatTask : DefaultTask() {
    ...
    fun run() {
        ...
        // The git ref of a downloaded api.txt, or the artifact coordinates of one generated from published classes.
        val ref = sdkFiles.firstNotNullOfOrNull { file ->
            file.useLines { lines -> lines.firstOrNull { it.startsWith("// Source: ") } }?.let { source ->
                when {
                    "firebase-android-sdk/" in source -> source.substringAfter("firebase-android-sdk/").substringBefore('/')
                    " of " in source -> source.substringAfterLast(" of ")
                    else -> null
                }
            }
        } ?: "unknown"
