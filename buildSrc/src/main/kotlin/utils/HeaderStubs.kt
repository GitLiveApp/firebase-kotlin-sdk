...
fun Project.stripHeaderStubs(
    ...
    tasks.withType(KotlinCompile::class.java).configureEach {
        ...
        doLast {
            ...
            val stubs = stubDirs.flatMap { dir -> dir.walkTopDown().filter { it.isFile && it.extension == "class" }.toList() }
                .filter { it.relativeTo(destination).invariantSeparatorsPath !in kept }
    ...
    tasks.withType(Jar::class.java).configureEach {
        ...
        exclude { element -> !element.isDirectory && packageDirs.any { element.path.startsWith("$it/") } && element.path !in kept }
    }
}
