 ...
 * [fromInstant] and [toKotlinInstant]. The `Parcelable` members are not mirrored.
 *
...
public expect class Timestamp(seconds: Long, nanoseconds: Int) : Comparable<Timestamp> {
    /** Android/JVM only: the `Timestamp(Date)` and `Timestamp(Instant)` constructors; use [fromInstant] from common code. */
