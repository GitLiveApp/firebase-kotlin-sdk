 ...
 * `Any?` overload). On Android [context] must be an `android.content.Context`; on Apple platforms, JS and wasmJs it is ignored.
 */
public expect fun Firebase.getApps(context: Any? = null): List<FirebaseApp>

/**
 * The default [FirebaseOptions] of the platform's configuration, mirroring `FirebaseOptions.fromResource(Context)` from
 * the Firebase Android SDK as a top-level function with the `Context` widened to `Any?` so that common code can call it.
 * On Android they come from the resources generated by the Google Services Gradle plugin from `google-services.json`,
 * on Apple platforms from `GoogleService-Info.plist` (the [context] is ignored); JS has no default configuration and
