...
public actual class FirebaseInstallations internal constructor(public val js: Installations) {
    ...
    public actual fun getId(): Task<String> = task({ jsGetId(js) }) { it.toKotlinString() }

    public actual fun getToken(forceRefresh: Boolean): Task<InstallationTokenResult> = task({ jsGetToken(js, forceRefresh) }) { JsInstallationTokenResult(it.toKotlinString()) }

    public actual fun delete(): Task<Nothing?> = task({ jsDelete(js) }) { null }

    public actual fun registerFidListener(listener: FidListener): FidListenerHandle {
        val unsubscribe = rethrow { jsOnIdChange(js) { listener.onFidChanged(it.toKotlinString()) } }
...
private inline fun <T : JsAny?, R> task(start: () -> Promise<T>, crossinline result: (T) -> R): Task<R> {
    val source = TaskCompletionSource<R>()
    try {
        start().then(
            onFulfilled = {
                source.setResult(result(it))
                null
            },
            onRejected = {
                source.setException(it.toInstallationsException())
                null
            },
        )
    } catch (e: Throwable) {
...
private fun Throwable.toInstallationsException(): FirebaseInstallationsException = when (this) {
    is FirebaseInstallationsException -> this
    is JsException -> installationsException(errorCode(), thrownValue?.let { jsGet(it, "message") }.toKotlin()?.toString() ?: message, this)
    else -> installationsException(null, message, this)
}
