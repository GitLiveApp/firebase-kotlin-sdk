...
def compat_report_coverage(moddir):
    ...
    path = os.path.join(ROOT, moddir, 'api', 'android-sdk-compat.txt')
    if not os.path.exists(path):
        return None
    for line in open(path):
        m = re.match(r'# (\d+)% of (\d+) public members available \((\d+) identical, (\d+) mapped', line)
        if m:
            return int(m.group(1)), int(m.group(3)) + int(m.group(4)), int(m.group(2))
...
def main():
    ...
    for api, moddir in MODULES.items():
        if args.modules and api not in args.modules:
            continue
        report = compat_report_coverage(moddir)
        if report:
            pct, hit_n, total_n = report
            results[moddir] = pct
            print(f'{moddir:22s} {hit_n:4d}/{total_n:4d} = {pct:3d}%  (api/android-sdk-compat.txt)')
            continue
