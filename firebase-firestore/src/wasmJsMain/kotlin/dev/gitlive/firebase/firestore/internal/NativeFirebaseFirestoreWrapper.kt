...
internal actual class NativeFirebaseFirestoreWrapper internal constructor(
    ...
    internal actual constructor(native: NativeFirebaseFirestore, databaseId: String?) : this(
        { settings ->
            settings?.let {
                NativeFirebaseFirestore(initializeFirestore(native.js.app, it.js, databaseId))
            } ?: native
        },
        databaseId,
    )
    ...
    actual suspend fun clearPersistence() = rethrow { clearIndexedDbPersistence(js).awaitUnit() }
    ...
    actual suspend fun disableNetwork() {
        rethrow { dev.gitlive.firebase.firestore.externals.disableNetwork(js).awaitUnit() }
    }
