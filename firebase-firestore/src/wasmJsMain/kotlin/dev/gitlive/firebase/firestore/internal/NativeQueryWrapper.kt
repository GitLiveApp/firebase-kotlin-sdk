...
internal actual open class NativeQueryWrapper internal actual constructor(actual open val native: NativeQuery) {
    ...
    actual suspend fun get(source: Source) = rethrow { QuerySnapshot(js.get(source).awaitValue()) }
    ...
    actual suspend fun count(): Long = rethrow {
        val data = getCountFromServer(js).awaitValue().data()
        (jsGet(data, "count").toKotlin() as Number).toLong()
    }
    ...
    private suspend fun aggregateDouble(aggregateField: AggregateField): Double? = rethrow {
        val snapshot = getAggregateFromServer(js, json("result" to aggregateField)).awaitValue()
        (jsGet(snapshot.data(), "result").toKotlin() as? Number)?.toDouble()
    }
    ...
    private fun Filter.toQueryConstraint(): QueryConstraint = when (this) {
        ...
        is Filter.Field -> {
            val value = constraintValue(constraint)
    ...
    private fun constraintValue(constraint: WhereConstraint): JsAny? = when (constraint) {
        is WhereConstraint.ForNullableObject -> constraint.value.toJs()
        is WhereConstraint.ForObject -> constraint.value.toJs()
        is WhereConstraint.ForArray -> jsArrayOf(constraint.values.toList())
    }
    ...
    actual fun startAfter(vararg fieldValues: Any) = rethrow {
        query(
            js,
            dev.gitlive.firebase.firestore.externals.startAfter(*fieldValues.map { it.toJs() }.toTypedArray()),
        ).wrapped
    }
    ...
    actual fun startAt(vararg fieldValues: Any) = rethrow {
        query(
            js,
            dev.gitlive.firebase.firestore.externals.startAt(*fieldValues.map { it.toJs() }.toTypedArray()),
        ).wrapped
    }
    ...
    actual fun endBefore(vararg fieldValues: Any) = rethrow {
        query(
            js,
            dev.gitlive.firebase.firestore.externals.endBefore(*fieldValues.map { it.toJs() }.toTypedArray()),
        ).wrapped
    }
    ...
    actual fun endAt(vararg fieldValues: Any) = rethrow {
        query(
            js,
            dev.gitlive.firebase.firestore.externals.endAt(*fieldValues.map { it.toJs() }.toTypedArray()),
        ).wrapped
    }
