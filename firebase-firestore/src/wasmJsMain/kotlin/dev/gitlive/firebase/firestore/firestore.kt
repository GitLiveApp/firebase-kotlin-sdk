...
public actual data class FirebaseFirestoreSettings(
    ...
    internal val js: JsAny get() {
        val settings = jsObject()
        jsSet(settings, "ssl", sslEnabled.toJs())
        jsSet(settings, "host", host.toJs())
        val localCache = when (val cache = cacheSettings) {
            is LocalCacheSettings.Persistent -> persistentLocalCache(
                json("cacheSizeBytes" to cache.sizeBytes),
            )
            is LocalCacheSettings.Memory -> {
                val garbageCollectorSettings = when (val garbageCollectorSettings = cache.garbaseCollectorSettings) {
                ...
                memoryLocalCache(json("garbageCollector" to garbageCollectorSettings))
            }
        }
        jsSet(settings, "localCache", localCache)
        return settings
    }
}
...
public actual class QuerySnapshot(internal val js: JsQuerySnapshot) {
    public actual val documents: List<DocumentSnapshot>
        get() = js.docs.toList().map { DocumentSnapshot(NativeDocumentSnapshotWrapper(it)) }
    public actual val documentChanges: List<DocumentChange>
        get() = js.docChanges().toList().map { DocumentChange(it) }
...
internal fun errorToException(cause: JsException): FirebaseFirestoreException = jsErrorToException(cause.thrownValue, cause)

internal fun errorToException(error: JsAny): FirebaseFirestoreException = jsErrorToException(error, Exception(jsGet(error, "message")?.toKotlin()?.toString()))

private fun jsErrorToException(error: JsAny?, cause: Throwable): FirebaseFirestoreException = (error?.let { jsGet(it, "code") }?.toKotlin()?.toString() ?: cause.message ?: "")
    .lowercase()
    .let {
        when {
            "cancelled" in it -> FirebaseFirestoreException(cause, FirestoreExceptionCode.CANCELLED)
            "invalid-argument" in it -> FirebaseFirestoreException(cause, FirestoreExceptionCode.INVALID_ARGUMENT)
            "deadline-exceeded" in it -> FirebaseFirestoreException(cause, FirestoreExceptionCode.DEADLINE_EXCEEDED)
            "not-found" in it -> FirebaseFirestoreException(cause, FirestoreExceptionCode.NOT_FOUND)
            "already-exists" in it -> FirebaseFirestoreException(cause, FirestoreExceptionCode.ALREADY_EXISTS)
            "permission-denied" in it -> FirebaseFirestoreException(cause, FirestoreExceptionCode.PERMISSION_DENIED)
            "resource-exhausted" in it -> FirebaseFirestoreException(cause, FirestoreExceptionCode.RESOURCE_EXHAUSTED)
            "failed-precondition" in it -> FirebaseFirestoreException(cause, FirestoreExceptionCode.FAILED_PRECONDITION)
            "aborted" in it -> FirebaseFirestoreException(cause, FirestoreExceptionCode.ABORTED)
            "out-of-range" in it -> FirebaseFirestoreException(cause, FirestoreExceptionCode.OUT_OF_RANGE)
            "unimplemented" in it -> FirebaseFirestoreException(cause, FirestoreExceptionCode.UNIMPLEMENTED)
            "internal" in it -> FirebaseFirestoreException(cause, FirestoreExceptionCode.INTERNAL)
            "unavailable" in it -> FirebaseFirestoreException(cause, FirestoreExceptionCode.UNAVAILABLE)
            "data-loss" in it -> FirebaseFirestoreException(cause, FirestoreExceptionCode.DATA_LOSS)
            "unauthenticated" in it -> FirebaseFirestoreException(cause, FirestoreExceptionCode.UNAUTHENTICATED)
            "unknown" in it -> FirebaseFirestoreException(cause, FirestoreExceptionCode.UNKNOWN)
            else -> {
                println("Unknown error code in ${jsonStringify(error)}")
                FirebaseFirestoreException(cause, FirestoreExceptionCode.UNKNOWN)
            }
        }
    }

internal fun entriesOf(jsObject: JsAny): List<Pair<String, Any?>> {
    val entries = jsObjectEntries(jsObject)
    return buildList {
        for (index in 0 until entries.length) {
            val entry = entries[index]!!
            add(entry[0].toKotlin() as String to entry[1].toKotlin())
        }
    }
}

internal fun mapOf(jsObject: JsAny): Map<String, Any?> = entriesOf(jsObject).toMap()
