...
public actual class StorageReference(internal val js: dev.gitlive.firebase.storage.externals.StorageReference) {
    ...
    public actual suspend fun putFile(file: File, metadata: FirebaseStorageMetadata?): Unit = rethrow { uploadBytes(js, file, metadata?.toStorageMetadata()).awaitUnit() }
...
internal inline fun <R> rethrow(function: () -> R): R {
    try {
        ...
        throw e.toFirebaseStorageException()
    }
}

internal fun JsException.toFirebaseStorageException(): FirebaseStorageException {
    val error = thrownValue
    val code = (error?.let { jsGet(it, "code") }?.toKotlin()?.toString() ?: message ?: "").lowercase()
    println("Unknown error code in ${jsonStringify(error)}")
    return FirebaseStorageException(code, this)
}

internal fun StorageError.toFirebaseStorageException(): FirebaseStorageException {
    val code = (jsGet(this, "code")?.toKotlin()?.toString() ?: "").lowercase()
    val message = jsGet(this, "message")?.toKotlin()?.toString()
    println("Unknown error code in ${jsonStringify(this)}")
    return FirebaseStorageException(code, Exception(message))
}
...
internal fun UploadMetadata.toFirebaseStorageMetadata(): FirebaseStorageMetadata {
    ...
    return storageMetadata {
        ...
        customMetadata = sdkMetadata.customMetadata?.let { metadata ->
            jsObjectKeys(metadata).toList().associate { keyJs ->
                val key = keyJs.toKotlinString()
                key to (jsGet(metadata, key)?.toKotlin()?.toString().orEmpty())
            }
        }.orEmpty().toMutableMap()
    }
}

internal fun FirebaseStorageMetadata.toStorageMetadata(): JsAny = json(
